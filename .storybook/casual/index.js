...
import { addToDate, subtractFromDate, moveTimeToDate, weekEnd, now } from "/app/utils/date";
...
casual.define( "upcomingDate", () => addToDate( now(), { minutes: casual.integer( 1, config.eventThresholds.upcoming ) } ) );
casual.define( "futureDate", () => addToDate( now(), { minutes: casual.integer( 1, 20160 ) } ) );
casual.define( "pastDate", () => subtractFromDate( now(), { minutes: casual.integer( config.eventThresholds.past + 1, 20160 ) } ) );
...
casual.define( "eventTimes", tense => {

    const allDay = casual.integer( 0, 10 ) > 8;
    const today = now();
    ...
    const endTime = allDay ? addToDate( startTime, { minutes: casual.integer( 30, 24 * 60 - 1 ) } ) : null;
...
casual.define( "eventSummary", ( { tense, Categories } ) => {
    return {
        ...
        "categories": casual.categories( Categories ),
        "featured": casual.integer( 0, 3 ) === 0,

        "multimedia": casual.multimedia,

        ...casual.eventTimes( tense )
    }
} );
